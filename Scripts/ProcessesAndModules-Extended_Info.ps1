...
$Import = Import-Csv -Path "$LogFile" -Delimiter "`t" -Encoding UTF8 | Where-Object { $_."Found On Disk" -eq "Yes" } | Group-Object "Company Name" | Select-Object @{ Name = 'Company Name'; Expression = { if ($_.Name) { $_.Name }
		else { 'N/A' } } }, Count, @{ Name = 'PercentUse'; Expression = { "{0:p2}" -f ($_.Count / $Total) } } | Sort-Object Count -Descending
...
$Total = (Import-Csv -Path "$LogFile" -Delimiter "`t" | Where-Object { $_."Found On Disk" -eq "Yes" } | Measure-Object).Count
$Import = Import-Csv -Path "$LogFile" -Delimiter "`t" -Encoding UTF8 | Where-Object { $_."Found On Disk" -eq "Yes" } | Group-Object "Digital Signature" | Select-Object @{ Name = 'Digital Signature'; Expression = { if ($_.Name) { $_.Name }
		else { 'N/A' } } }, Count, @{ Name = 'PercentUse'; Expression = { "{0:p2}" -f ($_.Count / $Total) } } | Sort-Object Count -Descending
...
# Found On Disk (Stats)
$Total = (Import-Csv -Path "$LogFile" -Delimiter "`t" | Measure-Object).Count
$Import = Import-Csv -Path "$LogFile" -Delimiter "`t" -Encoding UTF8 | Group-Object "Found On Disk" | Select-Object @{ Name = 'Found On Disk'; Expression = { if ($_.Name) { $_.Name }
		else { 'N/A' } } }, Count, @{ Name = 'PercentUse'; Expression = { "{0:p2}" -f ($_.Count / $Total) } } | Sort-Object Count -Descending
